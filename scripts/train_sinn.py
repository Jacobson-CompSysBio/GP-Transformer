...
from utils.dataset import GxE_Dataset, compute_leo_val_envs, normalize_env_categorical_mode
...
SINN_DATA_PATH = "data/maize_data_2014-2023_vs_2024_v2/"
ENVWISE_LOSS_TERMS = {"envmse", "envpcc", "envspearman", "triplet"}
...
class SINNGxEModel(nn.Module):
    """Two-prong GxE model with explicit additive finetune support."""
    ...
    def __init__(
        ...
        **encoder_kwargs,
        ...
        if interaction_type == "self_attn":
            self.gxe_blocks = nn.ModuleList([
                TransformerBlock(config) for _ in range(n_gxe_layers)
            ])
            self.ln_f = nn.LayerNorm(config.n_embd)
            self.ge_head = nn.Sequential(
                nn.Linear(config.n_embd, config.n_embd // 4),
                nn.GELU(),
                nn.Linear(config.n_embd // 4, 1),
            )
            nn.init.normal_(self.ge_head[-1].weight, std=0.01)
            nn.init.zeros_(self.ge_head[-1].bias)
        elif interaction_type == "bilinear":
            rank = interaction_rank if interaction_rank is not None else max(16, config.n_embd // 8)
            self.interaction_rank = rank
            self.g_interaction = nn.Linear(config.n_embd, rank, bias=False)
            self.e_interaction = nn.Linear(config.n_embd, rank, bias=False)
            self.interaction_vector = nn.Parameter(torch.empty(rank))
            self.interaction_bias = nn.Parameter(torch.zeros(1))
            nn.init.normal_(self.g_interaction.weight, std=0.02)
            nn.init.normal_(self.e_interaction.weight, std=0.02)
            nn.init.normal_(self.interaction_vector, std=0.02)
        else:
            raise ValueError(f"Unsupported SINN interaction_type: {interaction_type}")
    ...
    def _interaction_forward(self, g_enc, e_enc):
        if self.interaction_type == "self_attn":
            e_tok = e_enc.unsqueeze(1)
            tokens = torch.cat([g_enc, e_tok], dim=1)
            for block in self.gxe_blocks:
                tokens = block(tokens)
            tokens = self.ln_f(tokens)
            cls_repr = tokens[:, 0]
            return self.ge_head(cls_repr)

        g_cls = g_enc[:, 0]
        g_latent = self.g_interaction(g_cls)
        e_latent = self.e_interaction(e_enc)
        return (
            (g_latent * e_latent * self.interaction_vector).sum(dim=-1, keepdim=True)
    ...
    def forward(self, x, return_aux=False):
        ...
        g_aux = self.g_aux_head(g_cls)
        e_aux = self.e_aux_head(e_enc)
        year_aux = self.year_mean_head(e_enc)
        ge_pred = self._interaction_forward(g_enc, e_enc)

        if self.output_mode == "additive":
            pred = year_aux + g_aux + e_aux + ge_pred
        else:
            pred = ge_pred
...
def parse_sinn_args():
    ...
    p.add_argument("--decomp_source", type=str, default="split",
                   choices=["path", "split"],
                   help="Use a fixed decomposition path or fit labels from the observed pre-test split")
    p.add_argument("--decomp_test_year", type=int, default=2024,
                   help="Rows with Year < decomp_test_year define the observed pre-test universe")
    # architecture
    ...
    p.add_argument("--sinn_interaction", type=str, default="bilinear",
                   choices=["self_attn", "bilinear"],
                   help="Interaction form for SINN ge/finetune phases")
    p.add_argument("--interaction_rank", type=int, default=None,
                   help="Low-rank width for bilinear SINN interaction")
    # finetune phase options
    p.add_argument("--finetune_lr", type=float, default=None,
                   help="Lower LR for fine-tuning phase (defaults to 1e-5 unless --lr is explicitly passed)")
    ...
    p.add_argument("--e_select_metric", type=str, default="mse_minus_corr",
                   choices=["mse", "mse_minus_corr"],
...
# -- Data loading -------------------------------------------------
def _loss_terms(loss_name: str) -> set[str]:
    return {term.strip().lower() for term in str(loss_name).split("+") if term.strip()}
...
def _build_split_decomposition_payload(args, observed_mask, verbose=False):
    df = load_training_frame(SINN_DATA_PATH)
    return fit_decomposition_from_df(
        df,
...
def build_datasets(args, rank):
    ...
    observed_mask = None
    if args.leo_val:
        x_meta = pd.read_csv(os.path.join(SINN_DATA_PATH, "X_train.csv"), usecols=["id", "Env"])
        leo_val_envs = compute_leo_val_envs(
            x_meta,
            test_year=args.decomp_test_year,
            val_fraction=args.leo_val_fraction,
            seed=args.seed,
        )
        df_for_mask = load_training_frame(SINN_DATA_PATH)
        observed_mask = (df_for_mask["Year"] < args.decomp_test_year) & ~df_for_mask["Env"].isin(leo_val_envs)
    else:
        df_for_mask = load_training_frame(SINN_DATA_PATH)
        observed_mask = df_for_mask["Year"] < (args.decomp_test_year - 1)

    if args.decomp_source == "split":
        decomposition_payload = _build_split_decomposition_payload(
            args,
            observed_mask=observed_mask,
    ...
    val_ds = GxE_Dataset(
        ...
        leo_val=args.leo_val,
        leo_val_envs=leo_val_envs if leo_val_envs is not None else train_ds.leo_val_envs,
        decomposition_path=decomposition_path,
        decomposition_payload=decomposition_payload,
    )
...
# -- Model construction -------------------------------------------
def build_model(args, config, device, rank):
    ...
    if phase == "g":
        ...
        model = SINNGxEModel(
            ...
            interaction_type=args.sinn_interaction,
            interaction_rank=args.interaction_rank,
            output_mode="ge" if phase == "ge" else "additive",
            encoder_type="dense",
        ).to(device)

        if phase == "ge":
            _load_encoder_weights(model, args, device, rank)
...
def _load_encoder_weights(model, args, device, rank):
    ...
    if args.g_ckpt:
        ...
        g_head_state = {
            k.replace("head.", "", 1): v
            for k, v in g_state.items() if k.startswith("head.")
        }
        head_missing, head_unexpected = ([], [])
        if g_head_state:
            head_missing, head_unexpected = model.g_aux_head.load_state_dict(g_head_state, strict=False)
        if is_main(rank):
            ...
            if head_missing:
                print(f"[SINN]   G head missing keys: {head_missing}")
            if head_unexpected:
                print(f"[SINN]   G head unexpected keys: {head_unexpected}")
    ...
    if args.e_ckpt:
        ...
        e_head_state = {
            k.replace("head.", "", 1): v
            for k, v in e_state.items() if k.startswith("head.")
        }
        head_missing, head_unexpected = ([], [])
        if e_head_state:
            head_missing, head_unexpected = model.e_aux_head.load_state_dict(e_head_state, strict=False)
        if is_main(rank):
            ...
            if head_missing:
                print(f"[SINN]   E head missing keys: {head_missing}")
            if head_unexpected:
                print(f"[SINN]   E head unexpected keys: {head_unexpected}")
...
def _selection_metric(phase, args, val_mse, val_env_pcc):
    if phase == "finetune":
        return val_env_pcc, "val_env_pcc", "max"
    if phase == "e" and args.e_select_metric == "mse_minus_corr":
        corr_term = 0.0 if math.isnan(val_env_pcc) else val_env_pcc
        return val_mse - args.e_corr_weight * corr_term, "val_select_metric", "min"
    return val_mse, "val_mse", "min"
...
def train_phase(args, model, train_loader, val_loader, train_sampler,
    ...
    if is_main(rank):
        ...
        wandb.config.update({
            ...
            "decomp_test_year": args.decomp_test_year,
            "sinn_interaction": args.sinn_interaction,
            "interaction_rank": args.interaction_rank,
        ...
        run.define_metric("val/select_metric", step_metric="epoch")
    ...
    # training state
    _, _, selection_direction = _selection_metric(phase, args, float("inf"), float("-inf"))
    best_val_metric = -float("inf") if selection_direction == "max" else float("inf")
    last_improved = 0
    iter_num = 0
    t0 = time.time()
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            ...
            val_mse = F.mse_loss(full_preds, full_targets).item()
        ...
        if is_main(rank):
            metric, metric_name, selection_direction = _selection_metric(
                phase,
                args,
                val_mse,
                val_env_pcc,
            ...
            if improved:
                ...
                ckpt = {
                    ...
                    "config": {
                        ...
                        "decomp_source": args.decomp_source,
                        "decomp_test_year": args.decomp_test_year,
                        "sinn_interaction": getattr(ddp_model.module, "interaction_type", args.sinn_interaction),
                        "interaction_rank": getattr(ddp_model.module, "interaction_rank", args.interaction_rank),
...
def main():
    ...
    if is_main(rank):
        ...
        if args.decomp_source == "split":
            print(f"[SINN] Decomposition source: split (years < {args.decomp_test_year})")
        else:
            print(f"[SINN] Decomposition path: {args.decomp_path}")
        if args.sinn_phase in ("ge", "finetune"):
            print(f"[SINN] Interaction: {args.sinn_interaction}")
        if args.g_ckpt:
